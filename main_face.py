...
if __name__ == '__main__':
    ...
    src_encoder = init_model(net=SythnetEncoder(inputc = cfg.inputc, nf = cfg.d_input_dims),
                             restore=cfg.src_encoder_restore)
    src_classifier = init_model(net=SythnetClassifier(nf = cfg.d_input_dims, ncls = cfg.ncls),
                                restore=cfg.src_classifier_restore)    
    tgt_classifier = init_model(net=SythnetClassifier(nf = cfg.d_input_dims, ncls = cfg.ncls),
                                restore=cfg.src_classifier_restore)
    tgt_encoder = init_model(net=SythnetEncoder(inputc = cfg.inputc, nf = cfg.d_input_dims),
    ...
    generator = init_model(net=SythnetGenerator(input_dims=cfg.d_input_dims, outputc = cfg.inputc),
                         restore=cfg.src_generator_restore)    
    discriminator = init_model(net=Discriminator_img(nc = cfg.inputc), restore=None)
