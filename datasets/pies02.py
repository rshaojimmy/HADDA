...
def get_pie02(train, get_dataset=False, batch_size=cfg.batch_size):
    ...
    pie02_dataset = PIE02(root=cfg.data_root,
                                 split='train' if train else 'test',
                                 transform=pre_process,
                                 download=False)

    if get_dataset:
        return pie02_dataset
    else:
        pie02_data_loader = torch.utils.data.DataLoader(
            dataset=pie02_dataset,
            batch_size=batch_size,
            shuffle=True)
        return pie02_data_loader