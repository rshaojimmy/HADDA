...
def get_pie37(train, get_dataset=False, batch_size=cfg.batch_size):
    ...
    pie37_dataset = PIE37(root=cfg.data_root,
                                 split='train' if train else 'test',
                                 transform=pre_process,
                                 download=False)

    if get_dataset:
        return pie37_dataset
    else:
        pie37_data_loader = torch.utils.data.DataLoader(
            dataset=pie37_dataset,
            batch_size=batch_size,
            shuffle=True)
        return pie37_data_loader