...
def get_pie09(train, get_dataset=False, batch_size=cfg.batch_size):
    ...
    pie09_dataset = PIE09(root=cfg.data_root,
                                 split='train' if train else 'test',
                                 transform=pre_process,
                                 download=False)

    if get_dataset:
        return pie09_dataset
    else:
        pie09_data_loader = torch.utils.data.DataLoader(
            dataset=pie09_dataset,
            batch_size=batch_size,
            shuffle=True)
        return pie09_data_loader