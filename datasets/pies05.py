...
def get_pie05(train, get_dataset=False, batch_size=cfg.batch_size):
    ...
    pie05_dataset = PIE05(root=cfg.data_root,
                                 split='train' if train else 'test',
                                 transform=pre_process,
                                 download=False)

    if get_dataset:
        return pie05_dataset
    else:
        pie05_data_loader = torch.utils.data.DataLoader(
            dataset=pie05_dataset,
            batch_size=batch_size,
            shuffle=True)
        return pie05_data_loader