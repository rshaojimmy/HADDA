...
def get_pie27(train, get_dataset=False, batch_size=cfg.batch_size):
    ...
    pie27_dataset = PIE27(root=cfg.data_root,
                                 split='train' if train else 'test',
                                 transform=pre_process,
                                 download=False)

    if get_dataset:
        return pie27_dataset
    else:
        pie27_data_loader = torch.utils.data.DataLoader(
            dataset=pie27_dataset,
            batch_size=batch_size,
            shuffle=True)
        return pie27_data_loader